...
#' Familia tipográfica utilizable por el dispositivo gráfico (ADR-025)
#'
#' La identidad visual define `Inter` (shared/theme/theme_bmk.R). Esa familia se
#' sirve al NAVEGADOR como webfont (`bslib::font_google`), y los gráficos Plotly
#' la resuelven ahí sin problema. Pero el indicador de bandwidth se dibuja en el
#' SERVIDOR con el dispositivo gráfico de R, que solo puede usar fuentes
#' instaladas en el sistema operativo. Si `Inter` no está instalada, el
#' dispositivo ya dibujaba con su fuente sustitutiva y, además, emitía un warning
#' por cada elemento de texto (50+ avisos en el smoke test).
#'
...
#' defecto del dispositivo, exactamente la que ya se usaba). No altera resultados
#' ni el aspecto del gráfico; solo evita la búsqueda fallida. No añade
#' dependencias: `systemfonts` se consulta únicamente si está instalado.
...
#' Tema de marca para los gráficos, con familia tipográfica resuelta (ADR-025)
#' @section Tipo: Auxiliar (presentación).
  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    output$bw_indicator <- shiny::renderPlot({
      ...
      ggplot2::ggplot() +
        ...
        .theme_plot() +   # ADR-025: familia tipográfica resuelta (evita 50+ avisos)
        ggplot2::theme(
