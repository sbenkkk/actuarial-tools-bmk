...
# --- Descargas ---
# ADR-023: el contrato de `downloads` es diagnostics / risk / model_summary, tal
# como documentan el README (tabla del View Model y sufijos de exportación), lo
# que implementa mod_tool.R y lo que montan los tres downloadHandler de la UI.
# La aserción anterior comprobaba `downloads$curve`, un bloque que no existe:
# `curve` es una estructura interna del motor (estimation$curve), no una descarga.
check(setequal(names(vm$downloads), c("diagnostics", "risk", "model_summary")),
      "descargas: diagnostics + risk + model_summary")
check(nrow(vm$downloads$risk) == 3L, "descarga riesgo: 3 filas")
check(nrow(vm$downloads$model_summary) == 5L + 2L * nrow(vm$downloads$risk),
      "descarga resumen del modelo: 5 campos fijos + 2 filas por nivel de VaR")
